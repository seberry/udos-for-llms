...
Located in: `corpus/<town_slug>/<YYYY-MM-DD>/<source_type>/phase2_adu_tables/normalized/table_XX-XX_normalized.json`

```typescript
interface NormalizedRow {
  source_row_index: number;    // Original row index from OCR extraction
  label: string;               // Letter: A, B, C, D, E, F, G, H (may be empty)
  parameter: string;            // Parameter name (e.g., "Lot area", "Front")
  value: string;               // The actual value (clean, no [1] [2] markers)
  notes?: string;              // Notes column if present in source
  is_inferred?: boolean;       // True if auto-generated/inferred
  is_header?: boolean;         // True if section header row
  page: number;               // Source PDF page number
  table_ref: string;           // e.g., "02-2"
  source_text: string;         // Raw OCR text for debugging
}

interface NormalizedTable {
  table_ref: string;          // e.g., "02-2"
  table_title: string;        // Full table title
  pages: number[];            // PDF pages containing this table
  normalized_at: string;      // ISO timestamp of normalization
  rows: NormalizedRow[];       // All rows in table
}
```

### Key Rules for Dimensional Standards

1. **Labels**: Standardized letters A-G across most tables (H for RMH special cases)
...
2. **Values must be clean**: 
   - No concatenated markers like "25 ft [1] 30%"
   - Each parameter gets its own row
   - Multi-part values use line breaks: "Entire Dev: 43,560 sq ft<br>Dwelling Site: 3,000 sq ft"

3. **Row structure**:
   - Each parameter = one row (no merged cells)
   - Section headers have `is_header: true`
   - Notes rows have `label: "Notes"` and span all columns

4. **Inferred vs Verified**:
   - `is_inferred: true` = auto-generated from OCR (may have errors)
   - `is_inferred: false | undefined` = manually verified or corrected data

### Normalized JSONL Schema

Located in: `corpus/<town_slug>/<YYYY-MM-DD>/<source_type>/phase2_adu_tables/normalized/table_XX-XX_rows.jsonl`

One JSON object per line, matching `NormalizedRow` schema above.

### Comparison Page HTML

Located in: `corpus/<town_slug>/<YYYY-MM-DD>/<source_type>/phase2_adu_tables/normalized/dimensional_standards_comparison.html`

- Side-by-side comparison of PDF images vs extracted data
- Generated by: `src/build_dimensional_standards_comparison.ts`
- Used for human verification of table extraction quality

### HTML Generation Workflow

1. **Source**: `table_XX-XX_normalized.json` (authoritative)
2. **Generate**: `npx tsx src/generate_html_from_normalized_json.ts`
3. **Output**: `table_XX-XX_normalized.html` and `table_XX-XX_rows.jsonl`
4. **Deploy**: Copy from `corpus/...` to `public/...` (manual step)
